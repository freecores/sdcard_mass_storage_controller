...
void Start()
{
  ...
  uart_init();	
  sd_card_0 = sd_controller_init(); //Initiate card
  // Send out something  over UART
	...
	//Set Bus width to 4, CMD55 followed by ACMD 6
	   ...
	   SD_REG(SD_ARG)=0x2;
		...
		while (  ((rtn_reg  &1)  !=1  ) ){
		    ...
		    SD_REG(BD_ISR) =0;
	//Read from addr 0 to another array
	...
	/*
	   	SD_REG(SD_COMMAND) = CMD17 | CICE | CRCE | RSP_48;  
		SD_REG(SD_ARG)=0;
		if (!sd_wait_rsp())
			 	uart_print_str("Go send failed :/!\n");
