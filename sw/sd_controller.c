...
/*
int sd_get_cid(sd_card *d) //ADD CRC check
 {
   uint32 rtn_reg=0;
   ...
   uart_print_str("cidreg rec");
    rtn_reg = SD_REG(SD_RESP4);
	

    d->cid_reg->mid = rtn_reg & CID_MID_MASK;
	d->cid_reg->oid= (rtn_reg & CID_OID_MASK)<<1;
   
	rtn_reg = SD_REG(SD_RESP3);
	d->cid_reg->pnm[0] = rtn_reg & CID_B1;
	d->cid_reg->pnm[1] = rtn_reg & CID_B2;
	d->cid_reg->pnm[2] = rtn_reg & CID_B3;
	d->cid_reg->pnm[3] = rtn_reg & CID_B4;
	  }
	return 1;
 }
*/
